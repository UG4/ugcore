...
#include <vector>
#include <queue>
#include "hanging_node_refiner_ir1.h"
...
////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////
//	implementation of HangingNodeRefiner_IR1
HangingNodeRefiner_IR1::
HangingNodeRefiner_IR1(IRefinementCallback* refCallback) :
	m_pGrid(NULL),
	m_aVertex(false),
...
HangingNodeRefiner_IR1::
HangingNodeRefiner_IR1(Grid& grid, IRefinementCallback* refCallback) :
	m_pGrid(NULL),
	m_aVertex(false),
	m_refCallback(refCallback)
{
...
HangingNodeRefiner_IR1::HangingNodeRefiner_IR1(const HangingNodeRefiner_IR1& hnr) : m_aVertex(false)
{
	assert(!"WARNING in HangingNodeRefiner_IR1: Copy-Constructor not yet implemented. Please use references!");
	throw(int(0));
}

HangingNodeRefiner_IR1::~HangingNodeRefiner_IR1()
{
	if(m_pGrid)
	{
...
void HangingNodeRefiner_IR1::assign_grid(Grid& grid)
{
	set_grid(&grid);
}

void HangingNodeRefiner_IR1::
...
void HangingNodeRefiner_IR1::grid_to_be_destroyed(Grid* grid)
{
	if(m_pGrid)
		set_grid(NULL);
}

void HangingNodeRefiner_IR1::clear_marks()
{
	m_selMarkedElements.clear();
}

void HangingNodeRefiner_IR1::mark_for_refinement(EdgeBase* e)
{
	assert(m_pGrid && "ERROR in HangingNodeRefiner_IR1::mark_for_refinement(...): No grid assigned.");

	mark(e);
}

void HangingNodeRefiner_IR1::mark_for_refinement(Face* f)
{
	assert(m_pGrid && "ERROR in HangingNodeRefiner_IR1::mark_for_refinement(...): No grid assigned.");
...
void HangingNodeRefiner_IR1::mark_for_refinement(Volume* v)
{
	assert(m_pGrid && "ERROR in HangingNodeRefiner_IR1::mark_for_refinement(...): No grid assigned.");
...
void HangingNodeRefiner_IR1::refine()
{	
	assert(m_pGrid && "ERROR in HangingNodeRefiner_IR1::refine(...): No grid assigned.");
...
void HangingNodeRefiner_IR1::collect_objects_for_refine()
{
//	Algorithm Layout:
...
void HangingNodeRefiner_IR1::
refine_edge_with_normal_vertex(EdgeBase* e)
{
//	the grid
	Grid& grid = *m_pGrid;
...
void HangingNodeRefiner_IR1::
refine_edge_with_hanging_vertex(EdgeBase* e)
{
	Grid& grid = *m_pGrid;
...
void HangingNodeRefiner_IR1::
refine_face_with_normal_vertex(Face* f)
{
...
//	we'll perform a regular refine
	...
	f->refine_regular(vFaces, &nVrt, vNewEdgeVertices, NULL, Vertex(), NULL);
...
void HangingNodeRefiner_IR1::
refine_face_with_hanging_vertex(Face* f)
{
	...
	switch(numVrts)
	{
		case 3:
			...
			//	refine the constrained tri
				VertexBase* tmpVrt;
				constrainedTri.refine(vFaces, &tmpVrt, &vNewEdgeVertices.front(), NULL, NULL);
			}
			break;
		case 4:
			{
			...
			//	refine the constrained quad
				VertexBase* tmpVrt;
				cdf.refine(vFaces, &tmpVrt, &vNewEdgeVertices.front(), hv, NULL);
			}
			break;
		default:
...
void HangingNodeRefiner_IR1::
refine_volume_with_normal_vertex(Volume* v)
{
	Grid& grid = *m_pGrid;
