...
//	swaps are neither allowed for crease edges nor for fixed edges
	if(pshMarks){
		if(pshMarks->get_subset_index(e) == RemeshingElementMarks::REM_FIXED ||
			pshMarks->get_subset_index(e) == RemeshingElementMarks::REM_CREASE)
			return false;
	}
...
//	splits are not allowed for fixed edges
	if(pshMarks){
		if(pshMarks->get_subset_index(e) == RemeshingElementMarks::REM_FIXED)
			return false;
		else if(pshMarks->get_subset_index(e) == RemeshingElementMarks::REM_CREASE)
			bCreaseEdge = true;
	}
...
//TODO:	project crease vertices onto creases only! Don't project fixed vertices
				if(shMarks.get_subset_index(*iter) != RemeshingElementMarks::REM_FIXED){
					// vector3 vNew;
