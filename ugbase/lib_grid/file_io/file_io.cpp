					 ...
					 const char* filename, TAPos& aPos)
{
	...
	if(strName.find(".ugx") != string::npos){
		 ...
		 strName=strName.append(std::to_string(pcl::ProRank()));
		 strName.append(".ugx");   
		#endif
		
		if(psh)
			return SaveGridToUGX(grid, *psh, strName.str_c(), aPos);
		else {
			SubsetHandler shTmp(grid);
			return SaveGridToUGX(grid, shTmp, strName.str_c(), aPos);
		}
	}
	         ...
	         strNname=strName.replace(found, 4, "");
                 ...
                 strName.append(".vtu");
		#endif
		return SaveGridToVTU(grid, psh, strName.str_c(), aPos);
	}
	else
