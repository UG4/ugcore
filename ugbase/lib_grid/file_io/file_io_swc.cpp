...
bool FileReaderSWC::load_file(const char* fileName)
{
    ...
    while (std::getline(inFile, line))
    {
       ...
       if (strs.size() != 7)
       {
        ...
        // type
    	int int_type = boost::lexical_cast<int>(strs[1]);
        swc_types::swc_type type = static_cast<swc_types::swc_type>(int_type);
        switch (type)
        {
...
bool FileReaderSWC::create_grid(Grid& g, ISubsetHandler* pSH, number scale_length) const {
