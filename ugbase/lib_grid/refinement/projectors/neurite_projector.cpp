...
)
{
    ...
    for (size_t j = 0; j < nPts; ++j)
    {
         ...
         UG_LOGN("secIt->endParam(): " << secIt->endParam);
         ...
         UG_LOGN("radInv: " << radInv);
         UG_LOGN("velNorm: " << velNorm);
         UG_LOGN("posPosSq: " << posNormSq);

         number w = qPoints[j].second;
         defectOut += integrandVal * w * (end-start);
         UG_LOGN("defectOut neurite: " << integrandVal * w * (end-start));
         VecScaleAdd(gradientOut, 1.0, gradientOut, w * (end-start), blub);
         ...
         /// TODO: Vary soma radius with parameter t (Add length of t to radius)
         UG_LOGN("radSoma: " << somaRadius);
         UG_LOGN("radInvSoma: " << radInvSoma);
         UG_LOGN("velSoma: " << velSoma);
         UG_LOGN("velSomaNorm: " << velSomaNorm);
         UG_LOGN("posNormSomaSq: " << posNormSomaSq);
         number gradIntegrandValSoma = radInvSoma * exp(-posNormSomaSq*radInvSoma*radInvSoma) * velSomaNorm;
         vector3 blub2;
         VecScale(blub2, posAxSoma, gradIntegrandValSoma);
         VecScaleAdd(gradientOut, 1.0, gradientOut, w * (end-start), blub2);

         defectOut += gradIntegrandValSoma * w * (end-start);
         UG_LOGN("defectOut soma:" << gradIntegrandValSoma * w * (end-start));
    }

    /// TODO: Adjust approximate defect with additional soma points, see Breit Diss Eq. 7.3 and following
    defectOut -= sqrt(PI)*exp(-1.0);

    UG_LOGN("defectOut: " << defectOut);
...
(
	...
	number range = 0;
	/// FIXME: This method generates a range far too large
