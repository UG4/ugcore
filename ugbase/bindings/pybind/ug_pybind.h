 ...
 * 	Defined using variadic templates. */
...
struct ExportedClass : public pybind11::class_<TClass...>
{
	...
	ExportedClass(const base_type &py) : base_type(py)
	{}
...
protected:
	...
	{
		base_type::def( pybind11::init<Ts...>() );
	};
...
protected:
	...
	void mydef()
	{
		if (type_list::length == 1) {
			return mydef1<type_list>();
		} else if (type_list::length == 3)
		{
			return mydef3<type_list>();
		}
	}
	...
	void mydef1()
	{
			typedef typename type_list::head T1;
			base_type::def( pybind11::init<T1>() );
	}
	template<typename type_list>
	void mydef3()
	{
		typedef typename type_list::head T1;
		typedef typename type_list::tail::head T2;
		typedef typename type_list::tail::tail::head T3;
		base_type::def( pybind11::init<T1, T2, T3>() );
	}
};
...
void RegisterCommon(RegistryAdapter& reg, std::string grp)
{
