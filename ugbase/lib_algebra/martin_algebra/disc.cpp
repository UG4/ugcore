...
//! Problem class for discretisation
class cProblem
{
public:
...
//!
//! Laplace Problem class for discretisation. 
//! Laplace u = 0 inside, u=1 on boundary, solution 1
...
//!
//! Laplace Problem class for discretisation. 
//! Laplace u = 4 inside, u=x^2+y^2 on boundary, solution u=x^2+y^2
class Problemf4_2d : public cProblem
{
public:
	virtual double solution(double x, double y, double z) 
	{
...
//!
//! Laplace Problem class for discretisation. 
//! Laplace u = 6 inside, u=x^2+y^2+z^2 on boundary, solution u=x^2+y^2+z^2
...
void discretization2d(myMatrix &A, myVector &vx, myVector &b, int NX, int NY, int NZ, bool full_stencil, int problemtype)
{
	cProblem *problem;
	if(problemtype == 0)
		problem = new Laplace;
	else
		problem = new Problemf4_2d;
...
void discretization3d(myMatrix &A, myVector &vx, myVector &b, int NX, int NY, int NZ, bool full_stencil, int problemtype)
{
	cProblem *problem;
	if(problemtype == 0)
		problem = new Laplace;
...
void discretization(myMatrix &A, myVector &vx, myVector &b, int NX, int NY, int NZ, bool full_stencil, int problemtype)
{
	if(flexamg_dimensions == 2)
		discretization2d(A, vx, b, NX, NY, NZ, full_stencil, problemtype);
	else
		discretization3d(A, vx, b, NX, NY, NZ, full_stencil, problemtype);
}