...
bool Matrix::create_matrix(int nrow, int ncol)
{
	int err = 0;
...
bool Matrix::set_values(int nrows, int* ncols, int* rows, int* cols, double* values)
{
	if(m_hypreA == NULL) return false;
	return !(bool)HYPRE_IJMatrixSetValues(m_hypreA, nrows, ncols, rows, cols, values);
}

bool Matrix::add_values(int nrows, int* ncols, int* rows, int* cols, double* values)
{
	if(m_hypreA == NULL) return false;
	return !(bool)HYPRE_IJMatrixAddToValues(m_hypreA, nrows, ncols, rows, cols, values);
}

bool Matrix::set_dirichletrows(int nrows, int* rows)
{
	if(m_hypreA == NULL) return false;
	int one_ncols;
	int* one_cols;
...
bool Matrix::finalize()
{
	return !(bool)HYPRE_IJMatrixAssemble(m_hypreA);
}

bool Matrix::printToFile(const char* filename)
{
	if(m_hypreA == NULL) return false;
	return !(bool)HYPRE_IJMatrixPrint(m_hypreA, filename);
}

HYPRE_IJMatrix Matrix::getStorage()
{
	return m_hypreA;
}
