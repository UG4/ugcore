...
#ifndef __ORDERING_STRATEGIES_METAGRAPH_WEIGHTED_MATRIX_GRAPH__
#define __ORDERING_STRATEGIES_METAGRAPH_WEIGHTED_MATRIX_GRAPH__
...
class WeightedMatrixGraph : public IMetaGraph<G_t>{
public:
	typedef IMetaGraph<G_t> baseclass;

	WeightedMatrixGraph(matrix_type& A){
		...
		for(unsigned i = 0; i < rows; i++){
			for(typename matrix_type::row_iterator conn = A.begin_row(i); conn != A.end_row(i); ++conn){
				if(conn.value() != 0.0 && conn.index() != i){ //TODO: think about this!!
					...
					error();
	#endif
					boost::add_edge(i, conn.index(), w, baseclass::g);
				}
			}
		}
	}
