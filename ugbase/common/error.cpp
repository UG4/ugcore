...
{
	std::stringstream ss;
	if(static_cast<const std::bad_alloc*>(pex))
	...
	else if(static_cast<const std::bad_cast*>(pex))
	...
	else if(static_cast<const std::bad_exception*>(pex))
		ss << "bad_exception exception: " << pex->what() << "\nException thrown by unexpected handler.";
	else if(static_cast<const std::runtime_error*>(pex))
		ss << "runtime_error exception: " << pex->what();
	else if(static_cast<const std::logic_error*>(pex))
	{
		if(static_cast<const std::out_of_range*>(pex))
			ss << "out_of_range exception: " << pex->what();
		else if(static_cast<const std::length_error*>(pex))
