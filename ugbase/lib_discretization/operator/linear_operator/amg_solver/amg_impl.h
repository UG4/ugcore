...
{	
	P.create(A.row_size(), iNrOfCoarse);
#ifdef FLEXAMG
	...
	for(size_t i=0; i < A.row_size(); i++)
	{
		...
		{	
			...
			double diag = amg_diag_value(A.getDiag(i));
...
//! @param	newIndex		newIndex of coarse Node i on next coarser level
//! @param	iNrOfCoarse		nr of coarse nodes on this level
//! @param posInConnections		array of size A.row_size() for speedup of neighbor-neighbor-calculation inited with -1.
template<typename Matrix_type, typename Vector_type>
...
#endif
		cout << "Pass " << pass << ": ";
		for(size_t i=0; i<A.row_size() && unassigned > 0; i++)
		{
...
{
	amg_nodeinfo *grid = new amg_nodeinfo[A.row_size()];
	
	bool bTiming=true;
	cout << "Creating level " << level << ". (" << A.row_size() << " nodes)" << endl;
	stopwatch SW;
	...
	maxheap<amg_nodeinfo> PQ(A.row_size(), grid);
	
	std::vector<bool>	coarse(A.row_size());
	int unassigned = A.row_size();
	
	int *newIndex = new int[A.row_size()];
	memset(newIndex, -1, sizeof(int)*A.row_size());
	
	int *posInConnections = new int[A.row_size()];
	memset(posInConnections, -1, sizeof(int)*A.row_size());
	cout << "A.totalNrOfConnections = " << A.getTotalNrOfConnections() << endl;
	cgraph graph(A.row_size());
...
//#endif
	...
	cout << "AH: nnz: " << nnz << " Density: " << double(nnz)/(double(AH.row_size())*double(AH.row_size()))*100.0 << "% nnz/n: " << nnz/(double)AH.row_size() << endl;
	cout << "Coarsening rate: " << (100.0*AH.row_size())/(A.row_size()) << "%" << endl;
...
//	cout << R << endl;
	...
	if(this->A[0]->row_size() < AMG_WRITE_MATRICES_MAX)
	{
		cout << "write matrices";
...
bool amg<Matrix_type, Vector_type>::init(const Matrix_type& A_)
{
	...
	while(i< max_levels-1)
	{
		...
		vec3[i] = new Vector_type (A[i]->row_size(), "AMG:tempvec 3");
		vec3[i]->level = i;		
		i++;
	}
	
	int nrOfUnknowns = block_vector_traits< typename Vector_type::entry_type >::nrOfUnknowns;
	cout << "Creating level " << i << " (" << A[i]->row_size() << " nodes, total " << A[i]->row_size()*nrOfUnknowns << " unknowns)" << endl << "Using Direct Solver on Matrix " 
	<< A[i]->row_size()*nrOfUnknowns << "x" << A[i]->row_size()*nrOfUnknowns << ". ";
	...
	for(int i=0; i<used_levels; i++)
	{
		...
		totallength += A[i]->row_size();
	}	
	
	cout << "Operator Complexity: " << nnzs/A[0]->getTotalNrOfConnections() << " grid complexity: " << totallength/A[0]->row_size() << endl << endl;
