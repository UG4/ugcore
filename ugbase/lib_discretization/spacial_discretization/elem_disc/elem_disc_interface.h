...
class IElemDisc{
	...
	public:
		...
		{
			using std::vector;
			typedef void (IElemDisc<TAlgebra>::*ExportFunc)(vector<data_type>& val, vector<vector<data_type> >& deriv,
			...
			return (this->*(vDataExport[nr][m_id]))(val, deriv, pos, u, compDeriv);
		};
	...
	protected:
		...
		template <typename data_type, typename position_type>
		std::vector<std::vector<void (IElemDisc<TAlgebra>::*)(	std::vector<data_type>&, std::vector<std::vector<data_type> >&,
