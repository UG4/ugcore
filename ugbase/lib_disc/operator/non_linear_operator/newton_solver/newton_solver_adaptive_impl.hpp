...
#ifndef NEWTON_SOLVER_ADAPTIVE
#define NEWTON_SOLVER_ADAPTIVE

#include <iostream>
#include <sstream>

#include "nweton_solver_adaptive.hpp"
...
Newton_Solver_Adaptive<TAlgebra>::
Newton_Solver_Adaptive(SmartPtr<ILinearOperatorInverse<vector_type> > LinearSolver,
...
template <typename TAlgebra>
Newton_Solver_Adaptive<TAlgebra>::
Newton_Solver_Adaptive(SmartPtr<IOperator<vector_type> > N) :
	m_spLinearSolver(NULL),
	m_spConvCheck(new StdConvCheck<vector_type>(10, 1e-8, 1e-10, true)),
	m_spLineSearch(NULL),
	m_N(NULL),
...
template <typename TAlgebra>
Newton_Solver_Adaptive<TAlgebra>::
Newton_Solver_Adaptive(SmartPtr<IAssemble<TAlgebra> > spAss) :
	m_spLinearSolver(NULL),
	m_spConvCheck(new StdConvCheck<vector_type>(10, 1e-8, 1e-10, true)),
	m_spLineSearch(NULL),
	m_N(NULL),
...
template <typename TAlgebra>
void Newton_Solver_Adaptive<TAlgebra>::
...
template <typename TAlgebra>
bool Newton_Solver_Adaptive<TAlgebra>::init(SmartPtr<IOperator<vector_type> > N)
{
	NEWTON_PROFILE_BEGIN(NewtonSolver_init);
	m_N = N.template cast_dynamic<AssembledOperator<TAlgebra> >();
	if(m_N.invalid())
		UG_THROW("Newton_Solver_Adaptive: currently only works for AssembledDiscreteOperator.");
...
template <typename TAlgebra>
bool Newton_Solver_Adaptive<TAlgebra>::prepare(vector_type& u)
{
...
template <typename TAlgebra>
bool Newton_Solver_Adaptive<TAlgebra>::apply(vector_type& u)
{
	NEWTON_PROFILE_BEGIN(Newton_Solver_Adaptive);
//	increase call count
	m_dgbCall++;

//	Check for linear solver
	if(m_spLinearSolver.invalid())
		UG_THROW("Newton_Solver_Adaptive::apply: Linear Solver not set.");
...
//	Set dirichlet values
	...
	UG_CATCH_THROW("Newton_Solver_Adaptive::prepare: Prepare of Operator failed.");
...
// 	Compute first Defect
	try{
		...
		m_N->apply(*spD, u);
		NEWTON_PROFILE_END();
	}UG_CATCH_THROW("Newton_Solver_Adaptive::apply: Computation of Start-Defect failed.");
...
//	loop iteration
	...
	// 	Compute Jacobian
		...
		}UG_CATCH_THROW("Newton_Solver_Adaptive::apply: Initialization of Jacobian failed.");
	...
	// 	Init Jacobi Inverse
		try{
			...
			{
				UG_LOG("ERROR in 'Newton_Solver_Adaptive::apply': Cannot init Inverse Linear "
						"Operator for Jacobi-Operator.\n");
				return false;
			}
			NEWTON_PROFILE_END();
		}UG_CATCH_THROW("Newton_Solver_Adaptive::apply: Initialization of Linear Solver failed.");
	...
	// 	Solve Linearized System
		try{
			...
			{
				UG_LOG("ERROR in 'Newton_Solver_Adaptive::apply': Cannot apply Inverse Linear "
						"Operator for Jacobi-Operator.\n");
				return false;
			}
			NEWTON_PROFILE_END();
		}UG_CATCH_THROW("Newton_Solver_Adaptive::apply: Application of Linear Solver failed.");
	...
	//	store convergence history
		...
		// 	Line Search
			if(m_spLineSearch.valid())
			{
				...
				{
					UG_LOG("ERROR in 'Newton_Solver_Adaptive::apply': "
...
//				if( ! (*m_newtonUpdater)() )
		...
		}UG_CATCH_THROW("Newton_Solver_Adaptive::apply: Line Search update failed.");
...
template <typename TAlgebra>
void Newton_Solver_Adaptive<TAlgebra>::print_average_convergence() const
{
...
template <typename TAlgebra>
size_t Newton_Solver_Adaptive<TAlgebra>::num_newton_steps() const
{
	return m_vLinSolverCalls.size();
}

template <typename TAlgebra>
int Newton_Solver_Adaptive<TAlgebra>::num_linsolver_calls(size_t call) const
{
	return m_vLinSolverCalls[call];
}

template <typename TAlgebra>
int Newton_Solver_Adaptive<TAlgebra>::num_linsolver_steps(size_t call) const
{
	return m_vTotalLinSolverSteps[call];
}

template <typename TAlgebra>
double Newton_Solver_Adaptive<TAlgebra>::average_linear_steps(size_t call) const
{
	return m_vTotalLinSolverSteps[call] / ((double)m_vLinSolverCalls[call]);
}

template <typename TAlgebra>
int Newton_Solver_Adaptive<TAlgebra>::total_linsolver_calls() const
{
	int allCalls = 0;
...
template <typename TAlgebra>
int Newton_Solver_Adaptive<TAlgebra>::total_linsolver_steps() const
{
	int allSteps = 0;
	for(size_t call = 0; call < m_vLinSolverCalls.size(); ++call){
...
template <typename TAlgebra>
double Newton_Solver_Adaptive<TAlgebra>::total_average_linear_steps() const
{
...
template <typename TAlgebra>
void Newton_Solver_Adaptive<TAlgebra>::clear_average_convergence()
{
...
template <typename TAlgebra>
void Newton_Solver_Adaptive<TAlgebra>::write_debug(const vector_type& vec, std::string name)
{
...
template <typename TAlgebra>
void Newton_Solver_Adaptive<TAlgebra>::write_debug(const matrix_type& mat, std::string name)
{
//	add call count to name
	char ext[20]; snprintf(ext, 20, "_call%03d", m_dgbCall);
...
template <typename TAlgebra>
std::string Newton_Solver_Adaptive<TAlgebra>::config_string() const
{
	std::stringstream ss;
	ss << "Newton_Solver_Adaptive\n";

	ss << " LinearSolver: ";
	if(m_spLinearSolver.valid()) {
		ss << ConfigShift(m_spLinearSolver->config_string()) << "\n";
	} else {
		ss << " NOT SET!\n";
	}

	ss << " ConvergenceCheck: ";
	if(m_spConvCheck.valid()) {
		ss << ConfigShift(m_spConvCheck->config_string()) << "\n";
	} else {
		ss << " NOT SET!\n";
	}
	...
	ss << " LineSearch: ";
	if(m_spLineSearch.valid()) {
		ss << ConfigShift(m_spLineSearch->config_string()) << "\n";
	} else {
		ss << " not set.\n";
	}

	if(m_reassembe_J_freq != 0) {
		ss << " Reassembling Jacobian only once per " << m_reassembe_J_freq << " step(s)\n";
	}
	return ss.str();
}
