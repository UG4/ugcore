...
// 	each scv is associated to one corner of the element
	...
	//	store associated node
		...
		UG_LOG("rRefElem.num(dim-1,i,1) " << m_vSCV[i].numCorners << "dim " << dim << "\n");
...
// 	compute global informations for scvf
	for(size_t i = 0; i < num_scvf(); ++i)
	{
		...
		ElementNormal<face_type0,dim>(m_vSCVF[i].Normal,vCornerCoords);// face_type0 identical to scvf type
	}
...
// 	compute size of scv
	for(size_t i = 0; i < num_scv(); ++i)
	{
		...
		UG_LOG("-------------------------\n");
		UG_LOG("nc(" << i << ")=" << m_vSCV[i].numCorners << "\n");
		if (m_vSCV[i].numCorners-1==dim){
		     m_vSCV[i].Vol = ElementSize<scv_type0,dim>(m_vSCV[i].vGloPos);
		     UG_LOG("vol "<< i << " = " << m_vSCV[i].Vol << "\n");
		     ElementNormal<face_type0, dim>(m_vSCV[i].Normal, m_vSCV[i].vGloPos);
		} else { // m_vSCV[i].numCorners-2==dim , only possible in 3d
		     m_vSCV[i].Vol = ElementSize<scv_type1,dim>(m_vSCV[i].vGloPos);
		     ElementNormal<face_type1, dim>(m_vSCV[i].Normal, m_vSCV[i].vGloPos);
		};
...
//	loop requested subset
	...
	//	loop sides of element
		...
		//  fill BF with data from associated SCV
			...
			AveragePositions(bf.localIP,m_vSCV[side].vLocPos,m_vSCV[side].numCorners-1);
			AveragePositions(bf.globalIP,m_vSCV[side].vGloPos,m_vSCV[side].numCorners-1);
...
//template class DimCRFVGeometry<1, 2>;
//template class DimCRFVGeometry<1, 3>;

template class DimCRFVGeometry<2, 2>;
// template class DimCRFVGeometry<2, 3>;
