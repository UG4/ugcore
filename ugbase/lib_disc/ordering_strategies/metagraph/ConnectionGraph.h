...
#ifndef __UG__LIB_DISC__ORDERING_STRATEGIES_METAGRAPH_CONNECTION_GRAPH__
#define __UG__LIB_DISC__ORDERING_STRATEGIES_METAGRAPH_CONNECTION_GRAPH__
...
/*
	...
	The graph g has the same vertex set as the grid graph. E(g) = {{i, j}: i and j are contained in edges, faces or volumes}
*/
...
class ConnectionGraph : public IMetaGraph<G_t>{
public:
	typedef IMetaGraph<G_t> baseclass;

	//currently used for boost_{cuthill_mckee, minimum_degree}
	ConnectionGraph(SmartPtr<ug::ApproximationSpace<TDomain> > approx, unsigned grid_level){
		std::vector<SmartPtr<ug::DoFDistribution> > vDD = approx->dof_distributions();
		...
		UG_CATCH_THROW("[ConnectionGraph] No adjacency graph available.");
